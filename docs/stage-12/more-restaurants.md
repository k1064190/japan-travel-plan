...
- **39 new alt 식당 entries** (Day별 area + WebFetched Tabelog 별점/후기 수 한 번에)
  - Day1 (12): 신사이바시·도톤보리·난바 — 치토세, 메오토 젠자이, 하리주 스키야키, 혼케 오타코, 코가류, 호쿠쿄쿠세이 오므라이스, 진루이 미나 멘루이 FACTORY, 리로 커피, 긴큐에몬 도톤보리, 쿠쿠루 본점 등
...
검증:
- `npm run validate` → ✓ 170 places, 5 days validated
- `npm test` → 11 pass
...
- `data/places.json` (39 새 alt 식당 entries; UI 코드 변경 0 — 기존 Stage 8/9/10/11 인프라 그대로 활용)
- 패치 입력: `/tmp/more-restaurants/day{1,2,3,4}.json`
- 머지 스크립트: `/tmp/merge-more-restaurants.mjs`
- 관련 commit: `76c8457 data: add 39 more restaurant alt-candidates with WebFetched ratings`
...
### 검증
- `node scripts/validate-data.mjs` → ✓ 170 places, 5 days validated
